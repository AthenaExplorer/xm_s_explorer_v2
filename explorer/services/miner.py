...
from base.third import binghe_louts_sdk
...
class MinerService(object):
    ...
    def get_miner_day(cls, miner, date_str):
        ...
        worker_balance = WalletsService.get_is_all_wallet(miner.worker_id).value
        owner_balance = WalletsService.get_is_all_wallet(miner.owner_id).value
        post_balance = WalletsService.get_is_all_wallet(miner.post_id).value
    ...
    def sync_miner_lotus(cls):
        ...
        for miner in Miners.objects(actual_power__gt=Decimal128("0")).no_dereference().all():
            result_louts = binghe_louts_sdk.BingheLoutsBase().bill_to_miner_no(miner.miner)
