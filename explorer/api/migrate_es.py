import datetime
from xm_s_common.utils import datetime_to_height, height_to_datetime
from flask import request, current_app
from xm_s_common.third.binghe_sdk import BingheEsBase, BingheBase
...
from xm_s_common.utils import _d
...
def sync_all_message():
    ...
    while height < now_height:
        ...
        messages_list = []
        height += 1
        result = BingheEsBase().get_height_messages_back(height=height)
        messages = result.get('hits', [])
        if not messages:
            continue
        # 查询区块ID
        msg_cid_list = [per['_source'].get("msg_cid") for per in messages]
        block_result = BingheEsBase().get_block_by_message_ids(msg_cid_list)
...
def sync_all_overview():
    ...
    result = BingheBase().get_overview()
    if not result:
...
def sync_all_blocks():
    ...
    :return:
    """
    result = BingheBase().get_blocks(page_size=100, height=982609)
    print(result)
    page_size = 100
    now = datetime.datetime.now()
    ...
    result = BingheBase().get_blocks(page_size=page_size, height=start_index + page_size)
...
def sync_all_wallets():
    ...
    result = BingheEsBase().get_height_atpool_wallet(search_after=search_after)
...
def sync_all_miner():
    ...
    page_index = 0
    page_size = 100
    result = BingheBase().get_active_miners(page_index=page_index, page_size=page_size)
    if not result:
        return response_json(True)
